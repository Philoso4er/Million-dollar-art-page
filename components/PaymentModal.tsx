...
export default function PaymentModal({ pixelIds, onClose }: Props) {
  ...
  const reserve = async () => {
    ...
    if (!res.ok) {
      alert(data.error || "Failed to reserve pixels");
      setLoading(false);
      return;
    }
    ...
    // Auto-open Flutterwave checkout
    if (data.checkout) {
        ...
        {!reference ? (
          <>
            ...
            <div className="flex gap-3">
              ...
              <button
                disabled={loading}
                onClick={reserve}
                className="flex-1 py-2 rounded bg-green-600 font-semibold"
              >
          ...
          <>
            ...
            <a
              ...
              className="block text-center bg-yellow-600 py-2 rounded mb-4 font-bold"
            >
              Pay with Flutterwave
            </a>

            <h3 className="font-bold mb-2">Crypto Payments</h3>
            <p className="text-sm">Solana:</p>
            <p className="text-green-300 break-all mb-2">
              FcUQAQRfwo3mdUhchU1y4txpnPUEZEgYe98LesmAxdXM
            </p>

            <p className="text-sm">Ethereum / BSC / Polygon:</p>
